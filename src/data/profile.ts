    ...
    },
    {
      ...
      contentSections: [
        ...
        },
        {
          title: 'Как я работал',
          paragraphs: [
            'Вместе с PM, аналитиками и заказчиком разобрал слабые места старой системы: какие действия сотрудники выполняют ежедневно, где теряют время, что нужно редко, но должно быть доступно. Требования переводил в сценарии и прототипы, интерактивные макеты в Figma показывал заказчику и дирекции банка. Провёл [N] сессий с пользователями и заказчиком.',
            'Главные находки: сотрудники начинали поиск с тех данных, что есть под рукой (ИНН, фамилия), а форма требовала заполнять лишние обязательные поля; действия с документами были незаметны; назначение прав при большом числе объектов превращалось в ручную рутину.',
          ],
        },
        {
    ...
    },
    {
      ...
      contentSections: [
        ...
        },
        {
          title: 'Функционал',
          bullets: [
            'Дашборд компании/отдела с итоговой оценкой и динамикой по месяцам',
            'Карточка сотрудника с декомпозицией оценки по метрикам и списком задач, повлиявших на результат',
            'Раздел методологии с формулой расчёта и весами метрик',
            'Логика периодов: текущий / открытый прошлый / закрытый',
            'Администрирование источника данных: моки или реальный Kaiten API',
          ],
          paragraphs: [
            'Пользовательский сценарий. Руководитель открывает дашборд отдела → видит просевший показатель (например, дедлайны) → переходит в карточку сотрудника → видит, какие задачи снизили оценку → принимает решение (обратная связь сотруднику, пересмотр нагрузки и т.д.).',
            'Технические ограничения. Единственный источник данных — Kaiten API. При неполных данных по метрике система пересчитывает оценку только по доступным показателям и меняет сумму весов, не штрафуя сотрудника за метрику, которую нельзя посчитать.',
          ],
        },
        {
          title: 'Процесс решения проблемы и роль дизайнера в нём',
          paragraphs: [
            'Дизайн-методология. Работал модульным итеративным циклом: разбирал продукт на смысловые блоки (дашборд, карточка сотрудника, методология, периоды, администрирование) → для каждого блока формулировал сценарий, нужные данные и ограничения → превращал это в бриф для AI → генерировал интерфейс → проверял логику и UX, исправлял спорные решения → фиксировал модуль и переходил к следующему. Формального фреймворка (Design Thinking, Lean UX) не использовал — методология сложилась как рабочий процесс именно под связку «дизайнер + AI».',
            'Ресерч. Формального исследования с интервью и опросами не проводил. Источником требований стали собственный опыт работы с Kaiten и неформальные разговоры с коллегами о том, как сейчас на практике оценивают эффективность команды и с какими трудностями при этом сталкиваются. На основе этого определил, какие данные нужны руководителю: сроки, возвраты, план/факт трудозатрат, статусы задач, период, принадлежность к отделу.',
            'UX-артефакты. Структура продукта: дашборд, сотрудники, задачи, структура отделов, периоды, методология, администрирование — каждый раздел закрывает отдельный управленческий сценарий (посмотреть общую картину, проверить сотрудника, найти проблемные задачи, настроить период, понять формулу расчёта).',
            'Прототипирование. Рабочий прототип всех перечисленных экранов собирал через промпт-инжиниринг с AI: описывал сценарии, данные, состояния и ожидаемое поведение интерфейса, затем проверял результат и дорабатывал логику вручную, приводя интерфейс к единой продуктовой структуре.',
            'Качественные/количественные исследования. На этапе MVP тестирование ограничилось самостоятельной проверкой: логики расчёта, состояний интерфейса и сценариев на разных данных. Внешнего тестирования с руководителями отделов пока не было — это следующий шаг после того, как продукт начнут использовать в пилоте.',
          ],
        },
        {
